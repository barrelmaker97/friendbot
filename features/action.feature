...
Feature: Responding to Actions
	...
	Scenario: Send button is pressed
		...
		When we make a POST request at /action using ./example-data/actions/send.json
		Then we will get a 200 status code

	Scenario: Shuffle button is pressed
		Given friendbot is running
		When we make a POST request at /action using ./example-data/actions/shuffle.json
		Then we will get a 200 status code

	Scenario: Cancel button is pressed
		Given friendbot is running
		When we make a POST request at /action using ./example-data/actions/cancel.json
		Then we will get a 200 status code

	Scenario: Bad data is received
		Given friendbot is running
		When we make a POST request at /action using ./example-data/actions/bad.json
		Then we will get a 200 status code
